...
public class KBStats {
    ...
    public static void main(final String[] args) throws JSAPException, ClassNotFoundException, RocksDBException, IOException {
        ...
        if (!new File(kbMetadataFilename).exists())
            throw new IllegalArgumentException("No such file: " + kbMetadataFilename);
        ...
        for (final CallGraph callGraph : kb.callGraphs.values()) {
            ...
            if (totGraphs % 10000 == 0) {
                ...
                for (int i = 0; i < deflationBySize.length; i++)
                    if (deflationBySize[i].count() != 0)
                        System.out.println("Deflation by size [" + ((1L << i) - 1) + ".." + ((1L << i + 1) - 1) + "):" + deflationBySize[i].snapshot());
            }
