...
public class TrivialPredicateFactory implements PredicateFactory {
    ...
    protected JSONObject getMetadata(final MetadataSource source, final long callableGID) {
        switch (source) {
            case CALLABLE:
                return getCallableMetadata(callableGID);
            case MODULE:
                return getModuleMetadata(callableGID);
            case PACKAGE_VERSION:
                return getPackageVersionMetadata(callableGID);
            default:
                throw new NoSuchElementException("Unknown source");
        }
    }
