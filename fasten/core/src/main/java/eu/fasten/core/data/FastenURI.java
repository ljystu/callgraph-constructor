...
public class FastenURI implements Serializable {
    ...
    protected FastenURI(URI uri) {
        if (uri.getScheme() != null && !"fasten".equalsIgnoreCase(uri.getScheme()))
            throw new IllegalArgumentException("Scheme, if specified, must be 'fasten'");
        ...
        if (forgeProductVersion == null) rawForge = rawProduct = rawVersion = null;
            ...
            if (exclPos == -1) { // No forge
                ...
                if (productVersion.split("\\$")[0].indexOf('!') >= 0)
                    throw new IllegalArgumentException("More than one forge");
            }
        ...
        if (path.charAt(0) == '/') { // We have a namespace
            ...
            if (path.indexOf('/') != -1)
                throw new IllegalArgumentException("The entity part cannot contain a slash (namespaces must be always prefixed with a slash)"); // No slash
            rawNamespace = null;
            rawEntity = path;
        }
    ...
    protected void validateRawForge() {
        if (rawForge != null && rawForge.indexOf('$') >= 0)
            throw new IllegalArgumentException("Version / forge inverted or mixed");
    }

    protected void validateRawVersion() {
        if (rawVersion != null && rawVersion.indexOf('$') >= 0)
            throw new IllegalArgumentException("More than one version");
    }

    protected void validateRawProduct() {
        if (rawProduct != null && rawProduct.length() == 0)
            throw new IllegalArgumentException("The product cannot be empty");
    }

    protected void validateRawNamespace() {
        if (rawNamespace != null && rawNamespace.length() == 0)
            throw new IllegalArgumentException("The namespace cannot be empty");
    }

    protected void validateRawEntity() {
        if (rawEntity != null && rawEntity.length() == 0)
            throw new IllegalArgumentException("The entity part cannot be empty");
    ...
    public FastenURI relativize(final FastenURI u) {
        ...
        if (u.rawNamespace != null && !rawNamespace.equals(u.rawNamespace))
            return FastenURI.create("/" + u.rawNamespace + "/" + u.rawEntity);
