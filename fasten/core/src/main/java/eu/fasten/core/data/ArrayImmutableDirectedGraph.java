...
public class ArrayImmutableDirectedGraph implements DirectedGraph, Serializable {
    ...
    public static class Builder {
        ...
        public void addInternalNode(final long node) {
            if (graph.containsKey(node))
                throw new IllegalArgumentException("Node " + node + " is already in the node set");
        ...
        public void addArc(final long x, final long y) {
            ...
            if (numArcs * 2 + graph.size() >= Integer.MAX_VALUE - 8)
    ...
    public static ArrayImmutableDirectedGraph copyOf(final DirectedGraph graph, final boolean sorted) {
        ...
        graph.forEach(x -> {
            if (graph.isInternal(x)) builder.addInternalNode(x);
            else builder.addExternalNode(x);
        });
