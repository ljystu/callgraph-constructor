...
public class QueryDependentCentralitiesTest {
    ...
    public void testPageRankUniform() throws IOException {
        ...
        for (final long id : specialDirectedGraph.nodes())
            assertEquals(SPECIAL_GRAPH_UNIF[specialImmutableGraph.id2Node(id)], pr.get(id), 1E-6);
    }
    ...
    public void testPageRank1stHalf() throws IOException {
        ...
        pr = QueryDependentCentralities.pageRankParallel(specialDirectedGraph, queryNodes, 0.85);
        for (final long id : specialDirectedGraph.nodes())
            assertEquals(SPECIAL_GRAPH_1stHALF_S[specialImmutableGraph.id2Node(id)], pr.get(id), 1E-6);
    }
    ...
    public void testPageRank2ndHalf() throws IOException {
        ...
        pr = QueryDependentCentralities.pageRankParallel(specialDirectedGraph, queryNodes, 0.85);
        for (final long id : specialDirectedGraph.nodes())
            assertEquals(SPECIAL_GRAPH_2ndHALF_S[specialImmutableGraph.id2Node(id)], pr.get(id), 1E-6);
    }
    ...
    public void testWeightedPageRank() throws IOException {
        ...
        for (final long id : specialDirectedGraph.nodes())
            assertEquals(PAGE_RANK_WEIGHTED_RANK[specialImmutableGraph.id2Node(id)], katz.get(id), 1E-6);
    }


    @Test
    public void testKatz() throws IOException {
        @SuppressWarnings("deprecation") final AlphaCentrality<Long, LongLongPair> alpha = new AlphaCentrality<>(directedGraph, 0.0001, x -> queryNodes.contains(x) ? 1. / queryNodes.size() : 0., Integer.MAX_VALUE, 1E-7);
    ...
    public void testWeightedKatz() throws IOException {
        ...
        for (final long id : specialDirectedGraph.nodes())
            assertEquals(KATZ_WEIGHTED_RANK[specialImmutableGraph.id2Node(id)], katz.get(id), 1E-6);
    }
